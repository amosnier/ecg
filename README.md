...
Generate C++ header file from ARM SVD file.
...
```
In case no header file name is specified, the standard output stream is used by default.
...
### Why would we need new header files? Isn't that redundant with CMSIS and the libraries provided by micro-controllers vendor?
Yes, it is redundant.
...
[core_cm7.h](https://github.com/ARM-software/CMSIS/blob/master/CMSIS/Include/core_cm7.h) (and probably other CMSIS
...
first field in the `struct` above is always assumed to map bits 0 to 15, i.e. the least significant bits). This is
quite confusing and seems a little dangerous. The approach taken by `ecg` is to provide a bit-field runtime checking
function in the generated code (unfortunately, that cannot be done at compile time):
```C++
